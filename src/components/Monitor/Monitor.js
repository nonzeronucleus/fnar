...
import {getSelectedRoom, getCharactersInSelectedRoom, getDoorReleaseInSelectedRoom, getDoorReleaseStatusForSelectedRoom} from '../../redux/selectors';
...
export default () => {
    const mapState = useCallback(
        state => ({
          ...
          doorReleaseStatus: getDoorReleaseStatusForSelectedRoom(state),
        //   doorReleaseLock()
        }), []
    );

    const {room, charactersInRoom, doorRelease, doorReleaseStatus} = useMappedState(mapState);
    return (
        <>
            <RadarMap />
            ...
            <Scroller>
                ...
                {doorRelease !==null && <DoorRelase active={doorReleaseStatus === releaseButtonStatus.enabled}/>}
