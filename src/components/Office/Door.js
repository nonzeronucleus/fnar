...
import {getCharactersInRoom, isDoorOpen, isPlaying, getPower, isDoorReleaseTriggered} from '../../redux/selectors';
...
export default ({room, left}) => {
  ...
  const mapState = useCallback(
    state => ({
      ...
      doorReleaseTriggered: isDoorReleaseTriggered(state, room),
  ...
  const {charactersInRoom, open, playing, power, doorReleaseTriggered} = useMappedState(mapState);

  return <>
      {charactersInRoom.length > 0 && <Character character={charactersInRoom[0]} {...{left}} />}

      <DoorDisplay {...{left}} src={open ? doorOpen: doorClosed} onClick={() => playing && power> 0 && !doorReleaseTriggered && toggleDoor(!open)}/>
    </>
}
